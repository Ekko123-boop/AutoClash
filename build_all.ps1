# =====================================================================
# Cypher Fab Tools - Full End-to-End Build & Deployment Script
# Multi-Version Engine (Navisworks 2020 - 2026)
# =====================================================================
...
& $msbuild "CypherFabTools.csproj" -t:restore -p:Configuration=Release2023 -p:Platform=x64
if ($LASTEXITCODE -ne 0) { throw "NuGet restore failed." }

# 3. Build Multi-Version Plugin DLLs
Write-Host ">>> 1. Building Navisworks 2023 Engine (Release2023)..." -ForegroundColor Cyan
& $msbuild "CypherFabTools.csproj" -p:Configuration=Release2023 -p:Platform=x64
...
& $msbuild "CypherFabTools.csproj" -p:Configuration=Release2024 -p:Platform=x64
...
& $msbuild "CypherFabTools.csproj" -p:Configuration=Release2025 -p:Platform=x64
...
& $msbuild "CypherFabTools.csproj" -p:Configuration=Release2026 -p:Platform=x64
...
$staging = "$env:TEMP\cypherfab_bundle_staging"
...
Write-Host ">>> 6. Compiling Modern Standalone CypherFabTools_Installer.exe..." -ForegroundColor Cyan
& $msbuild "Installer\Installer.csproj" -p:Configuration=Release -p:Platform=x64
if ($LASTEXITCODE -ne 0) { throw "Installer build failed." }

$installerSource = "Installer\bin\Release\CypherFabTools_Installer.exe"
if (!(Test-Path $installerSource)) {
    throw "Installer binary not generated at $installerSource"
}

$destinations = @(
    "CypherFabTools_Installer.exe",
    "..\CypherFabTools_Installer.exe",
    "$env:USERPROFILE\Downloads\CypherFabTools_Installer.exe"
)
...
foreach ($dest in $destinations) {
    try {
        ...
        Write-Host " - Standalone Fab Installer ready at: $dest" -ForegroundColor Green
    } catch {
...
Write-Host ">>> 7. Deploying Multi-Version CypherFabTools.bundle to Navisworks ApplicationPlugins..." -ForegroundColor Cyan
$bundleDir = "$env:APPDATA\Autodesk\ApplicationPlugins\CypherFabTools.bundle"
...
Write-Host " - Multi-Version Fab Bundle deployed to: $bundleDir" -ForegroundColor Green

# 7. Clean Standalone User Plugins Directory (eliminating duplicate plugin loading)
Write-Host ">>> 8. Ensuring Clean User Plugins Directory..." -ForegroundColor Cyan
$legacyUserPlugins = @(
...
Write-Host "CYPHER FAB TOOLS BUILD & DEPLOYMENT 100% COMPLETE (2020-2026 READY)!" -ForegroundColor Green
Write-Host " - Standalone EXE Installer: CypherFabTools_Installer.exe" -ForegroundColor Green
Write-Host " - Bundle Folder:            CypherFabTools.bundle" -ForegroundColor Green
Write-Host " - Coexists with:            CypherGenericClash.bundle" -ForegroundColor Green
Write-Host "====================================================================" -ForegroundColor Green
